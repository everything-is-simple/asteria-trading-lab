...
def build_quant_report(equity_rows: list[dict[str, Any]], metrics: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "scope": {
            ...
            "unit": "point-unit: execution price point × recorded units, before any date-dependent share multiplier",
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "| 指标 | 数值 |",
        "|---|---:|",
        f"| 净利润 | {fmt_number(stats['net_profit'])} 点单位 |",
        ...
        f"| 资本基准 | {fmt_number(account['capital_base'])} 点单位 |",
        "",
        "## 交易统计",
        "",
        "| 指标 | 数值 |",
        ...
        f"| 毛利润 Gross Profit | {fmt_number(stats['gross_profit'])} 点单位 |",
        f"| 毛亏损 Gross Loss | {fmt_number(stats['gross_loss'])} 点单位 |",
        f"| Profit Factor | {fmt_number(stats['profit_factor'], 2)} |",
        f"| 平均盈利 | {fmt_number(stats['average_win'], 2)} 点单位 |",
        f"| 平均亏损 | {fmt_number(stats['average_loss'], 2)} 点单位 |",
        f"| 盈亏比 Payoff Ratio | {fmt_number(stats['payoff_ratio'], 2)} |",
        f"| 单笔期望值 | {fmt_number(stats['expectancy_per_trade'], 2)} 点单位 |",
        f"| 中位数单笔 | {fmt_number(stats['median_trade'], 2)} 点单位 |",
        f"| 单笔 PnL 标准差 | {fmt_number(stats['pnl_standard_deviation'], 2)} 点单位 |",
        ...
        f"| 最佳交易 | {stats['best_trade']['major_trade_id']} / {fmt_number(stats['best_trade']['pnl'])} 点单位 |",
        f"| 最差交易 | {stats['worst_trade']['major_trade_id']} / {fmt_number(stats['worst_trade']['pnl'])} 点单位 |",
        "",
