...
5. 用户指定的实战交易谱页组：`Z:\market-life-cycle\立花义正交易法-part4`
6. 章节原文 PDF / 图片页
7. OCR 文本，仅作检索辅助
...
| 级别 | 日线 |
| 交易单位 | 交易谱记录单位；part1-3 说明早期股票交易单位进入 1000 股口径，part4 又记录 PIONEER 自 1976-09-21 起交易单位改为 100 股。v0.1 回测先以 `point_unit = 价格点 × 记录单位` 列账，资金层必须后续按日期乘交易单位，不得全样本统一乘 1000。 |
| 价格 | 每日收盘价、交易价 |
...
| 原始写法 | v0.1 解释 |
|---|---|
| `—5` 作为买卖记录 | 买入 5 记录单位，增加多头或减少空头。 |
| `5—` 作为买卖记录 | 卖出 5 记录单位，增加空头或减少多头。 |
| `—5` 作为未平仓 | 多头 5 记录单位；真实股数需按当日交易单位换算。 |
| `10—` 作为未平仓 | 空头 10 记录单位；真实股数需按当日交易单位换算。 |
| `10—5` 作为未平仓 | 同时包含空头 10 记录单位、多头 5 记录单位。 |
| `—2 (600日圆)` | 以 600 日圆买 2 记录单位；真实股数需按当时交易单位换算。 |

因此，`position_raw` 中横杠左侧映射为 `gross_short`，横杠右侧映射为 `gross_long`。例如 `2—20` 应解析为 `gross_short = 2`、`gross_long = 20`。这条规则优先于早期“左侧/右侧”抽象描述。

`trade_raw` 与 `position_raw` 必须分离：同一个横杠写法在“买卖栏”表示当日交易动作，在“未平仓栏”表示库存状态。回测不得把 `10—2` 简化为净空 8 记录单位。
...
| `gross_long` | number | 多头总记录单位；来自 `position_raw` 横杠右侧。 | 从 `position_raw` 解析。 |
| `gross_short` | number | 空头总记录单位；来自 `position_raw` 横杠左侧。 | 从 `position_raw` 解析。 |
...
| `center_size` | number | 中心单候选记录单位。 | v0.1 主要人工标注。 |
| `add_on_size` | number | 围绕中心单追加的同向记录单位。 | 库存扩张可自动估算。 |
...
| 生成等待日 | 无交易但保留价格与持仓状态。 |
| 生成 `point_unit` 绩效 | 价格点 × 记录单位；资金层金额需按日期/交易单位表换算。 |
...
| 资金曲线和真实账户杠杆 | 原始资料不足以稳定定义资金基数；真实股票金额需在 `point_unit` 外按日期交易单位换算。 |
...
MALF 不管理仓位，不管理心理，不管理锁单，不决定中心单，不决定加码记录数量，不决定清仓模式。
