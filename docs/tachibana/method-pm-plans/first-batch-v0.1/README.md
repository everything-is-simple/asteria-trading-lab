...
10. **research agenda 层已接上** —— `--audit-first-batch-execution-policy-research-agenda` 会把 `execution_policy_research_prep` 的逐候选记录继续按 `candidate_constraint_type` 聚合为只读 `AShareExecutionPolicyResearchAgendaItem`：首批 6 条 prep 记录被整理为 3 条议题（`t1 / price_limit / suspension_resume`），其中 `t1 -> ready_for_research`、`price_limit -> await_additional_evidence`、`suspension_resume -> carry_forward_required`；它仍然只是研究议题入口，不是规则定义，也不是交易许可。
