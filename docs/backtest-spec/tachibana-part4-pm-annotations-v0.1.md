...
| 项 | v0.1 处理 |
|---|---|
| 回测金额层 | 继续用 `point_hand = 成交价点数 × 记录手数/单位`。 |
...
当前十五大交易报告可以保留为 `point_hand` 量化账本；但解释层必须降级为“交易谱回放统计”，不能宣称已经完整解释立花的全部意图。
...
4. 资金层单独建立日期依赖的 `share_multiplier`，不要污染现有 `point_hand` 绩效。
