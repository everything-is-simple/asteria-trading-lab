...
| `price_limit` | `await_additional_evidence` | 2 | 两条样本的涨跌停关键事实仍有 `unknown`，必须先补事实。 |
...
- `await_additional_evidence` 与 `carry_forward_required` 都是正常研究状态，不是系统错误。
...
- `agenda_status_counts.ready_for_research=1`
- `agenda_status_counts.await_additional_evidence=1`
