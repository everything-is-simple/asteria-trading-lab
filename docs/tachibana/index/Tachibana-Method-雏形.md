...
| [1976-12](../monthly/1976-12.md) `35 — -> 150 — -> 0` | 大仓位推进后以三次 `—50` 分批收束，显示利润保护不是单一退出点。 | 强化 `staged_execution` 与 `anti_vanity_profit_stability`，并提示回测需要单独处理大仓位退出。 |
| [1976-04](../monthly/1976-04.md) `— 10 -> 2 — 20 -> 4 — 5` | 面对双侧库存时，动作语义不应被简化成反手或预测反转。 | 强化 `accept_and_correct` 与 `staged_execution`，并把双侧库存交给 PM 层处理。 |
| [1976-03](../monthly/1976-03.md) `—12 -> 0 -> —10` | 清零后重新试探，说明认错/收束后仍可重新进入。 | 强化 `reset_after_clear` 与 `trend_probe_entry` 的边界。 |
| [1976-05](../monthly/1976-05.md) `4 — 5 -> 10 — 5 -> 10 — -> 0` | 双侧库存解除后可以继续单侧持有，再最终清仓。 | 强化 PM 与 Method 的分工：Method 解释动作语义，PM 保存库存状态。 |
...
- 1976 年已开始纳入定义校验，但除 1976-03/04/05/10/11/12 外多数月份尚未精读。
